...
function previewPhotos(input) {
  if (input.files && input.files.length > 0) {
    ...
    selectedPhotoFiles = [...selectedPhotoFiles, ...newFiles].slice(0, 2);
  }
...
function pdfAlumno() {
  ...
  if (achs.length) {
    ...
    doc.autoTable({
      startY: y, head: [['Logro', 'Descripción']],
  ...
  if (mine.length) {
    ...
    doc.autoTable({
      startY: y, head: [['Fecha', 'Grado', 'Acción', 'Pts', 'Descripción']],
...
function pdfRanking() {
  ...
  doc.autoTable({
    startY: y, head: [['#', 'Nombre', 'Equipo', 'Puntos', 'Logros', 'Estado']],
...
function pdfHistorial() {
  ...
  doc.autoTable({
    startY: y, head: [['Fecha', 'Alumno', 'Equipo', 'Grado', 'Acción', 'Pts', 'Descripción']],
...
function pdfEquipos() {
  ...
  doc.autoTable({
    startY: y, head: [['Equipo', 'Tema', 'Integrantes', 'Pts. totales', 'Promedio', 'En negativo']],
  ...
  Object.entries(teams).sort().forEach(([t, members]) => {
    ...
    doc.autoTable({
      startY: y2, head: [['Nombre', 'Puntos', 'Logros']],
...
function pdfAlertas() {
  ...
  if (neg.length) {
    doc.autoTable({
      startY: y, head: [['Nombre', 'Equipo', 'Puntos']], body: neg.map(([n, t]) => [n, t, points[n]]),
